...
<body class="bg-gray-100 font-sans">
    ...
    <main class="container mx-auto mt-8 p-4">
        ...
        <div id="add-course" class="">
            ...
            <form id="add-course-form" action="/teacher/course/addCourse" method="POST" enctype="multipart/form-data"> 
                <div class="mb-4">
        ...
        <div id="dashboard" class="hidden grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            <?php if (!empty($courses)):  ?>
                ...
                <div class="bg-white shadow-lg rounded-lg overflow-hidden cursor-pointer mb-4">
                    ...
                    <div class="p-4">
                        <h2 class="font-bold text-lg mb-2"><?php echo htmlspecialchars($course['title'] ?? ''); echo $course['courseID'];   ?></h2>
                        ...
                        <div class="flex justify-center space-x-4 mt-6">
                            ...
                            <form method="post" action="/teacher/course/delete">
                                <input type="text" class="hidden" name="dletedID" value="<?=$course['courseID'];?>">
                                ...
                                <button class="bg-transparent hover:bg-gray-200 rounded-full p-3" name="enrole" value="<?=$course['courseID']?>" title="View Enrollments" onclick="showEnrollmentList(<?php echo (int)($course['courseID']  ?? 0);?>)">
                    ...
                    <div id="enrollment-list<?= $course['courseID']?>" class="hidden fixed inset-0 w-full h-full bg-black bg-opacity-75 z-50 flex items-center justify-center">
                    <div class="relative bg-white rounded-lg w-full max-w-2xl mx-auto p-8">
                        <button class="absolute top-4 right-4 text-gray-600 hover:text-gray-800 text-xl" onclick="hideEnrollmentList(<?php echo (int)($course['courseID']  ?? 0);?>)">
                        ...
                        <ul class="list-disc list-inside text-gray-700 space-y-2">
                            <?php
                            $idii =  $course['courseID'];
                            $enrolles = $enr->enrollmentList($idii);
                            ?>
                            
                            <?php foreach($enrolles as $enrolle):?>

                            <li><?=$enrolle["lastName"] . " " . $enrolle["lastName"] . " " . $enrolle["enrollmentDate"]?></li>
                            <?php endforeach;?>
                        </ul>
                    </div>
                    </div>
        ...
        <div id="edit-course-popup" class="hidden fixed inset-0 w-full h-full bg-black bg-opacity-75 z-50 flex items-center justify-center">
            <div class="relative bg-white rounded-lg w-full max-w-2xl mx-auto p-8">
                ...
                <form action="/teacher/course/update" method="post">
                    ...
                    <div class="mb-6">
                        <input type="text" name="id" value="<?php var_dump($_SESSION["courseID"]);?>" class="hidden">
    ...
    <script>
        ...
        function showEnrollmentList(courseId) {
            console.log(courseId,99);
            
            document.getElementById("enrollment-list"+courseId).classList.remove('hidden');
            document.body.style.overflow = 'hidden';
            document.addEventListener('keydown', closeEnrollmentOnEsc);
        }

        function hideEnrollmentList(courseId) {
            document.getElementById("enrollment-list"+courseId).classList.add('hidden');
            document.body.style.overflow = 'auto';
        ...
        const textEditorContent = tinymce.get('description').getContent().trim();
        ...
        if (contentType === 'video' && !content) {
            errors.push('Please upload a video file.');
        } else if (contentType === 'text' && textEditorContent === '') {
            errors.push('Please enter content in the text editor.');
        }
